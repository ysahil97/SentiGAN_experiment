...
class Generator(object):
    def __init__(self, num_emb, vocab_dict, batch_size, emb_dim, num_units,
                 max_sequence_length, gen_num=0, learning_rate=0.01, reward_gamma=0.95,
                 ):
        ...
        with tf.variable_scope('placeholder'+str(self.gen_num)):
        ...
        with tf.variable_scope('embedding'+str(self.gen_num)):
            # batch_size major
            self.emb_x = tf.nn.embedding_lookup(self.g_embeddings, self.x)

        with tf.variable_scope('projection'+str(self.gen_num)):
        ...
        with tf.variable_scope("decoder"+str(self.gen_num)):
    ...
    def save_model(self, sess):
        self.saver.save(sess, 'save/ckpt/model_'+str(self.gen_num)+'.ckpt')
